 ...
 * a pathToCSV to write the measurements to a CSV file
...
public class YeastFluorGetter{
	 ...
	 * objects as null.
	 * 
	 * If the input string is null, the pathToCSV will be set to
	 * a "YeastFluorGetter<yyyy-MM-dd_HH-mm-ss-z>.csv file in the 
	 * current working directory.
	 */
	
	public YeastFluorGetter(ImageStack roiStack, ImageStack fluorStack, String pathToCSV,int set
		 ...
		 * 
		 * If pathToCSV is null, it will be set to
		 * a "YeastFluorGetter<yyyy-MM-dd_HH-mm-ss-z>.csv file in the 
		 * current working directory. 
		 */
		if(roiStack==null)
		...
		if(pathToCSV==null){
			...
			pathToCSV=s+File.separator+"YeastFluorGetter"+sdf.format(new Date())+".csv";
		}
	...
	public ImagePlus makeBinary(){
		return makeBinary(this.roiStack);
	}
	...
	*default table
	...
	public void writeHeaders(){
		writeHeaders(this.pathToCSV,this.rt);
	}
	...
	public void saveResults(int set,int pos){
		saveResults(this.rt,this.pathToCSV,set,pos);
	}
