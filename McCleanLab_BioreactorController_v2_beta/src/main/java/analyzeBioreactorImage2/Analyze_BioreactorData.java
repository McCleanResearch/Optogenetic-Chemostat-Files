...
public class Analyze_BioreactorData implements PlugIn {
	...
	private int constraint1Index=5;
	private double constraint1Min=1;
	private double constraint1Max=250;
	private int constraint2Index=6;
	private double constraint2Min=1;
	private double constraint2Max=250;
	
	
	@SuppressWarnings("unchecked")
	private boolean setup(){
		if(!IJ.showMessageWithCancel("","(Jan 24 test)Ensure that JHeatChart.jar is in the Fiji/Jars folder, \n\r then select the CSV file with your results"))
			return false;
		...
		if(maxIndex<6){
			...
			constraint1Index=maxIndex;
			constraint2Index=maxIndex;
		}
		...
		gd.addChoice("Constraint 1:", allVals.get(0), allVals.get(0)[constraint1Index]);
		gd.addNumericField("Constraint 1 min value (values below will be ignored,  not modified)",constraint1Min, 1);
		gd.addNumericField("Constraint 1 max value (values above will be ignored,  not modified)",constraint1Max, 1);
		gd.addChoice("Constraint 2:", allVals.get(0), allVals.get(0)[constraint2Index]);
		gd.addNumericField("Constraint 2 min value (values below will be ignored,  not modified)",constraint2Min, 1);
		gd.addNumericField("Constraint 2 max value (values above will be ignored,  not modified)",constraint2Max, 1);
		gd.showDialog();
		if (gd.wasCanceled()) {
			gd.dispose();
			return false;
			...
			this.constraint1Index=gd.getNextChoiceIndex();
			this.constraint1Min=gd.getNextNumber();
			this.constraint1Max=gd.getNextNumber();
			this.constraint2Index=gd.getNextChoiceIndex();
			this.constraint2Min=gd.getNextNumber();
			this.constraint2Max=gd.getNextNumber();
		}
	...
	public void run(String arg0) {
		...
		//The ith index of sampleSizes contains the number of ROIs that were found at the ith sample
		...
		List<double[]> statistics=new LinkedList<double[]>();
		fillStatistics(titles,statistics);
		drawPlots(titles,statistics,xAxis,yAxis);
		
		
		
		String csvFilePath=this.saveDir+this.title+sdf.format(new Date())+"_data.csv";
		
		saveDataInCSV(csvFilePath,titles,statistics);
		...
		IJ.showMessage("Plotting complete. The outputs have been saved in : \n\r"+this.saveDir+
				"\n\rThe parameters used were saved to the LOG file");
	}
	public Plot[] drawPlots(List<String> titles,List<double[]> statistics,String xAxis,String yAxis){
		Plot[] plots=new Plot[statistics.size()-1];
		Iterator<String> headerItr=titles.iterator();
		Iterator<double[]> dataItr=statistics.iterator();
		double[] times=dataItr.next();
		//ignore the first header, since it is the header ofr the xAxis values
		headerItr.next();
		for(int i=0;i<statistics.size()-1;i++){
			...
			plots[i]=new Plot(header,xAxis,yAxis,times,dataItr.next());
			ImagePlus img=plots[i].makeHighResolution(this.title+" "+header, 4, false, false);
			IJ.save(img, this.saveDir+this.title.replace("\\", "_").replace("/", "_")+" "+header+".tif");
			plots[i].show();
		}
		return plots;
	}
	public void fillStatistics(List<String> titles,List<double[]> statistics){
		double[] times=getDoubleArr(0,this.arrayOfHistograms.length*10,10);
		titles.add("Set");
		statistics.add(times);
		//Return here (typo is to draw attention)
		double[] sampleSizes=getSizeOfEachRow(this.histVals);
		//Should go from high to low percentiles, so that the last value is assumed to be backgorund intensity
		...
		for(int i=0;i<percentilesWanted.length;i++){
			titles.add(Double.toString(percentilesWanted[i])+"th percentile of Log10 of intensity");
			statistics.add(percentiles[i]);
			titles.add(Double.toString(percentilesWanted[i])+"th percentile of intensity");
			statistics.add(expPercentiles[i]);
		}
		for(int i=0;i<percentilesWanted.length-1;i++){
			double[] ratio=new double[percentiles[i].length];
			for(int j=0;j<percentiles[i].length;j++){
				ratio[j]=percentiles[i][j]/percentiles[percentilesWanted.length-1][j];
			}
			titles.add("Ratio of Log10("+Double.toString(percentilesWanted[i])+"th %)to Log10("+Double.toString(percentilesWanted[percentilesWanted.length-1])+"th %)");
			statistics.add(ratio);
		}
		for(int i=0;i<percentilesWanted.length-1;i++){
			double[] ratio=new double[expPercentiles[i].length];
			for(int j=0;j<expPercentiles[i].length;j++){
				ratio[j]=expPercentiles[i][j]/expPercentiles[percentilesWanted.length-1][j];
			}
			titles.add("Ratio of "+Double.toString(percentilesWanted[i])+"th % to "+Double.toString(percentilesWanted[percentilesWanted.length-1])+"th %");
			statistics.add(ratio);
		}
		titles.add("sample size");
		statistics.add(sampleSizes);
		
		Iterator<String> headerItr=titles.iterator();
		Iterator<double[]> dataItr=statistics.iterator();
		List<String> tmpTitles=new LinkedList<String>();
		List<double[]> tmpStats=new LinkedList<double[]>();
		int cols=statistics.size();
		for(int i=0;i<cols;i++){
			String header2p= headerItr.next();
			String header3p="3 point weighted rolling average of "+header2p;
			header2p="2 point weighted rolling average of "+header2p;
			double[] col=dataItr.next();
			tmpTitles.add(header2p);
			tmpStats.add(getRollingAverage(col,sampleSizes, 2));
			tmpTitles.add(header3p);
			tmpStats.add(getRollingAverage(col,sampleSizes, 3));
		}
		for(String title:tmpTitles)
			titles.add(title);
		for(double[] stat:tmpStats)
			statistics.add(stat);
		
	}
	public double[] getRollingAverage(double[] data,double[] weights, int period){
		if(data.length!=weights.length){
			IJ.error("McCleanLab Plot Bioreactor Results line, getRollingAverage() \n"
					+ "Cannot compute the rolling average if the number of data and weights differ");
			return data;		
		}
		if(period>data.length)
			period=data.length;
		int firstCenterIndexOfWindow=(period/2);
		int indecesBeforeCenter=firstCenterIndexOfWindow*-1;
		int indecesAfterCenter=period-(firstCenterIndexOfWindow+1);
		int lastCenteredIndex=(data.length-1)-indecesAfterCenter;
		double[] rollingAverage=new double[data.length];
		for(int i=firstCenterIndexOfWindow;i<=lastCenteredIndex;i++){
			double totSum=0;
			double totWeight=0;
			for(int j=indecesBeforeCenter;j<=indecesAfterCenter;j++){
				totSum=totSum+(data[(j+i)]*weights[j+i]);
				totWeight=totWeight+weights[j+i];
			}
			rollingAverage[i]=totSum/totWeight;			
		}
		return rollingAverage;
	}
	
	public void saveDataInCSV(String filePath,List<String> titles,List<double[]> data){
		try {
			...
			int rowsOfData=data.get(0).length;
			for(int i=0;i<rowsOfData;i++){
				String text="";
				boolean firstColumn=true;
				for(double[] column:data){
					if(firstColumn){
						text=Double.toString(column[i]);
						firstColumn=false;
						//enable this to work for columns of various sizes
					}else if(i>=column.length){
						text=text+",";
					}else{
						text=text+","+Double.toString(column[i]);
					}
				}
				writer.write(text+"\n");
			}
	...
	//returns a double[] in which the ith index of sampleSizes contains the number of ROIs that were found at the ith sample
	public double[] getSizeOfEachRow(List<Double[]> listOfDoubles){
		double[] sizes=new double[listOfDoubles.size()];
		int i=0;
		for(Double[] row:listOfDoubles){
			sizes[i]=(double)row.length;
			i++;
		}
		return sizes;
	}
	...
	public double[][] getPercentile(List<Double[]> listOfArrs,double[] percentiles){
		...
		for(Double[] arrOfDoubles:listOfArrs){
			...
			//IJ.log("Set "+Integer.toString(i)+" has these values: \n\r"+Arrays.toString(arr));
			...
			for(int j=0;j<percentiles.length;j++){
				...
				//IJ.log("Set "+Integer.toString(i)+" has these percentiles: \n\r"+Double.toString(percentileArr[j][i]));
			}
			i++;
		}
	...
	public double[][] getExponentialVals(double[][] vals){
		...
		for(int i=0;i<rows;i++)
			for(int j=0;j<columns;j++)
				expVals[i][j]=Math.pow(10.0,vals[i][j]);
		return expVals;
	}
	...
	public List<Double[]> getHistVals(){
		...
		for(String[] row:this.allVals){
			if(firstRow){ //if headers
				...
				histVal=new Double(Math.log10(histVal));
				...
				double constraint1=Double.parseDouble(row[this.constraint1Index]);
				double constraint2=Double.parseDouble(row[this.constraint2Index]);
				if(!(constraint1<this.constraint1Min
						||constraint1>this.constraint1Max
						||constraint2<this.constraint2Min
						||constraint2>this.constraint2Max))
	...
	public String toString(){
		String response="\n\r\n\rCSV file of source data: \n\r   "+this.csvPath+
				...
				"\n\r\n\rMaximum histogram value to bin: \n\r"+Math.pow(10.0,this.maxHistVal)+
				"\n\r\n\rMinimum histogram value to bin: \n\r"+Math.pow(10.0,this.minHistVal)+
				...
				"\n\r\n\rThe index of constraint 1 was: \n\r"+Integer.toString(this.constraint1Index)+
				"\n\r\n\rThe minimum value of constraint 1 was: \n\r"+Double.toString(this.constraint1Min)+
				"\n\r\n\rThe maximum value of constraint 1 was: \n\r"+Double.toString(this.constraint1Max)+
				"\n\r\n\rThe index of constraint 2 was: \n\r"+Integer.toString(this.constraint2Index)+
				"\n\r\n\rThe minimum value of constraint 2 was: \n\r"+Double.toString(this.constraint2Min)+
				"\n\r\n\rThe maximum value of constraint 2 was: \n\r"+Double.toString(this.constraint2Max)+
				"Constraint 1 was"+this.heatMapPlotPath+
				"\n\r\n\r*******IMPORTANT*******\n\r"
