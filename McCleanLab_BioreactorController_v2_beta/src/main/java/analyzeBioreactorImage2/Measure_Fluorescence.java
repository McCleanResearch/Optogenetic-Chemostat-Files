 ...
 */
public class Measure_Fluorescence implements PlugIn,ActionListener {
	...
	List<String> roiFilePaths=new LinkedList<String>();
	...
	public boolean setup() {
		...
		if(WindowManager.getImageTitles()==null||WindowManager.getImageTitles().length<2){
			IJ.showMessage("Open image stacks first, then reload this plugin");
			return false;
		}
		if (!IJ.showMessageWithCancel(
				"PRECONDITIONS",
				"1)This plugin works on Virtual Stacks and regular stacks that require <~60% of \n\r"
				...
				+ "This plugin assumes that the stack is sorted, with the \n\r"
						...
						+ "\n\r2) Ensure that cvMatch_Template plugin is available.")) {
			return false;
		}
		...
		gd.addChoice(
				...
				WindowManager.getImageTitles(), WindowManager.getCurrentImage()
						.getTitle());
		gd.addNumericField("Min cell area (with budding yeast, 1000 is good for 40x PC and 20 is good for 10x PC)", 1000, 3);
		gd.addNumericField("Max cell area (with budding yeast, 10000 is good for 40x PC and 500 is good for 10x PC)", 10000, 3);
		...
		gd.addChoice("Image analysis program", analysisOptions, analysisOptions[0]);
		gd.showDialog();
		if (gd.wasCanceled()) {
			gd.dispose();
			return false;
			...
			this.analysisOptionSelected=gd.getNextChoice();
		...
		//Redirect error messages to IJ.log()
	...
	public void run(String arg) {
		...
		for (int curSlice = 1; curSlice <= this.stackSize; curSlice++) {
			...
			if(curSlice%this.positions==0||curSlice==this.stackSize){
				...
				if(this.analysisOptionSelected.equals(OPTION3)){
					...
					if(this.analysisOptionSelected.equals(OPTION2)){
				...
				if(set%5==0&&set>0){
					String roiFilePath=this.dir+"RoiSetFromPreviousSetTo"+Integer.toString(set)+".zip";
					roiFilePaths.add(roiFilePath);						
					if(!YeastFluorGetter.saveRois(allRois,roiFilePath))
							IJ.log("Rois were not saved properly from the previous set to set: "+Integer.toString(set));
					allRois.clear();
				}
		...
		*/
		//12/22
		String roiFilePath=this.dir+"RoiSetFromPreviousSetTo"+Integer.toString((this.stackSize-1)/this.positions)+".zip";
		roiFilePaths.add(roiFilePath);
		if(!YeastFluorGetter.saveRois(allRois,roiFilePath))
			IJ.log("Rois were not saved properly from the previous set to set: "+Integer.toString((this.stackSize-1)/this.positions));
		allRois.clear();
		...
		for(String filePath:roiFilePaths)
		...
		rm.runCommand("Save", this.dir+"RoisFromAllSets.zip");
